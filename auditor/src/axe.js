...
export async function runAxe(url) {
  ...
  try {
    ...
    const weight = { critical: 10, serious: 6, moderate: 3, minor: 1, unknown: 1 };
    const penalty = Object.entries(countsByImpact).reduce((sum, [k, c]) => sum + (weight[k] || 1) * c, 0);
    const normalized = {
      countsByImpact,
    ...
    return { success: true, durationMs: Date.now()-start, raw: results, normalized };
  } catch (e) {
